	...
	Creap = (function() {
		...
		(function() {
			...
			(Loader = function() {
				 ...
				 * @see Creap.Content
				 * @param data {object|array<object>} Content data(s).
				 * @param data.path {string} File path.
				 * @param data.lib {string} Name of library object. (Maybe, named 'lib')
				 * @param data.root {string} Name of the root object in library object.
				 * @param data.images {string} Name of images object. (Maybe, named 'images')
				 * @param data.ss {string} Name of sprite sheet object. (Maybe, named 'ss')
				 * @param data.basePath {string} Basement path of aseets.
				 * @param callback {function} Callback when the file loaded.
				 * @return {Creap.Loader} Return a itself (can use method chaining).
				 */
				loadContent: {
		...
		(function() {
			...
			function initialize(p, c, w, upr, ugl) {
				 ...
				 */
				this.view = view;
				this.view.style.width = this.view.width + TAG_PX;
				this.view.style.height = this.view.height + TAG_PX;
			 ...
			 * @param [options.width=lib.properties.width] {number} Horizontal resolution of canvas.
			 * @param [options.usePreRender=true] {bool} Whether render bitmaps before starting the content.
			 * @param [options.useWebGL=true] {bool} Whether use webGL render.
			 */
			(Application = function(content, container, options) {
				 ...
				 * 
				 * @since 1.1.0
				 * @function Creap.Application#getPosition
				 * @return {Creap.Position}
				 */
				getPosition: {
					value: function() {
						return new Position(
		...
		/**
		 * @constructor Creap.Position
		 * @classdesc Class related to position data.
				...
				value: Object.defineProperties({}, {
					...
					fonts: {
						...
						set: function(obj) {
							...
							obj = obj || {};
							switch (webFontLoadedState) {
								case 0:
									var css = document.createElement('style');
									css.type = 'text/css';
									document.getElementsByTagName('head').item(0).appendChild(css);
									autoLoadCss = document.styleSheets.item(document.styleSheets.length - 1);
									
									webFontLoadedState = 1;
									for (i in obj) {
										willLoadFonts[i] = obj[i];
									}
									loader.load('http://ajax.googleapis.com/ajax/libs/webfont/1/webfont.js', function() {
										var v;
										webFontLoadedState = 2;
										for (i in obj) {
											willLoadFonts[i] = obj[i];
										}
										
										for (i in willLoadFonts) {
											v = willLoadFonts[i].match(/\..*$/);
											autoLoadCss.insertRule('@font-face {font-family: "' + i + '"; src: url(' + willLoadFonts[i] + ')' + ((v in FONT_EXT) ? '' : ' format("' + FONT_EXT[v] + '");') + '}');
		...
		(function() {
			...
			(function() {
				...
				(CreapData = function() {
					...
					/**
					 * Whether current path is to use as hole.
					 * 
					 * @member createjs.Graphics~CreapData#isHole {bool}
					 * @default false
					 */
					this.isHole = false;
				});
				...
				(Graphics = function(obj) {
					...
					moveTo: {
						value: function(x, y) {
							...
							if (this.containsPoint({x:x, y:y})) {
								this._creap.isHole = true;
							}
					...
					closePath: {
						value: function() {
							...
							if (this._creap.isHole) {
								this.addHole();
							}
							this._creap.isHole = false;
							return this;
						}
					},
					beginFill: {
