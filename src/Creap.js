...
(function() {
	...
	var PRE_MOUSE_EVENT = {
		mousedown: '___' + EVENT.mousedown,
		pressup: '___' + EVENT.pressup,
		pressmove: '___' + EVENT.pressmove,
		click: '___' + EVENT.click
	};
	
	var REV_MOUSE_EVENT = {
		___mousedown: EVENT.mousedown,
		___pressup: EVENT.pressup,
		___pressmove: EVENT.pressmove,
		___click: EVENT.click
	};
	...
	createjs = (function() {
		...
		(function() {
			...
			(DisplayObjectCreapData = function(obj) {
				...
				/**
				 * State of ancestor's display#interactive.
				 * 
				 * @member createjs~DisplayObjectCreapData#propagationalInteractive {bool}
				 */
				this.propagationalInteractive = false;
				...
				/**
				 * Changes the touch judgment of a child element having an entity.<br />
				 * Elements are recursively scanned to the bottom layer.
				 * 
				 * @function createjs~DisplayObjectCreapData#setInteractive
				 * @since 1.1.8
				 p @param flag {bool} Whether to accept touch event.
				 */
				setInteractive: {
					value: function(flag) {
						var c, i;
						for (i = 0; i < this.target.children.length; i++) {
							c = this.target.children[i];
							if (c instanceof MovieClip) {
								if (c.interactive) {
									continue;
								}
								c._creap.propagationalInteractive = flag;
								c._creap.setInteractive(flag);
							} else if (c instanceof Bitmap || c instanceof Sprite || c instanceof Shape || c instanceof Text) {
								c.interactive = flag;
							}
						}
					}
				...
				(DisplayObject = function() {
					...
					addEventListener: {
						value: function(type, func, isCapture) {
							if (type in CJS_MOUSE_EVENT) {
								...
								if (isAccurateTarget) {
									this._creap.propagationalInteractive = true;
									this._creap.setInteractive(true);
								}
							}
					...
					removeEventListener: {
						value: function(type, func, isCapture) {
							...
							if (isAccurateTarget) {
								if (type in CJS_MOUSE_EVENT && !this.parent._creap.propagationalInteractive) {
									for (var i in REV_MOUSE_EVENT) {
									...
									this._creap.propagationalInteractive = false;
									this._creap.setInteractive(false);
								}
							} else {
								this.interactive = false;
							}
						...
						*/
						...
						if (isAccurateTarget) {
							this._creap.on(CREAP_EVENT.attach, function() {
								if (!this.interactive) {
									this.interactive = this.parent._creap.propagationalInteractive;
								}
							});
						}
				...
				(MovieClip = function() {
					...
					initialize: {
						value: function(mode, startPosition, loop, labels) {
							...
							if (isAccurateTarget) {
								this._creap.on(CREAP_EVENT.attach, function() {
									if (!this.interactive) {
										this._creap.propagationalInteractive = this.parent._creap.propagationalInteractive;
									} else {
										this._creap.propagationalInteractive = true;
									}
									
									this._creap.setInteractive(this._creap.propagationalInteractive);
								});
							}
						}
					},
					...
					addChild: {
						value: function(child) {
							...
							var c = PIXI.Container.prototype.addChild.call(this, child);
							child._creap.emit(CREAP_EVENT.attach);
							return c;
						}
					},
					...
					addChildAt: {
						value: function(child, index) {
							...
							var c = PIXI.Container.prototype.addChildAt.call(this, child, Math.min(index + this._creap.onLayerCount, this.children.length));
							child._creap.emit(CREAP_EVENT.attach);
							return c;
						}
					},
